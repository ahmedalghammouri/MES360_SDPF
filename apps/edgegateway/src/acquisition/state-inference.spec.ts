...
describe('StateInferenceService', () => {
  ...
  const LINE = [
    { id: 'm1', sortOrder: 1 }, // Big Betti — filler
    { id: 'm2', sortOrder: 2 }, // Checkweigher
    { id: 'm3', sortOrder: 3 }, // Cartomac
  ...
  it('leaves a running machine alone when it has no PROCESSING signal', async () => {
    // Without a signal saying whether product is flowing there is no way to tell a
    // busy machine from an idle one, and guessing would invent external losses.
  ...
  // ── NCC's actual case ──────────────────────────────────────────────────────
  // Cartomac and Uni-Tech report Run Mode ON while starved: "ready, but no product
  ...
  it('requires ALL upstream machines to be stopped, not just one', async () => {
    // m1 is down but m2 is still running, so material is still arriving.
