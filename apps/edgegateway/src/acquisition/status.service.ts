 ...
 *
 * NCC's Euro-Pack Robot reports: steady ON = running, PULSING = stop mode,
...
export class StatusService {
  ...
  private async derive(tag: StatusTag, numeric: number): Promise<string | null> {
    if (tag.dataType === 'BOOL') {
      ...
      // Three states on one bit (NCC's Euro-Pack Robot):
      ...
      // starved — NCC's spec says "running/ready, INCLUDING ready with no product
