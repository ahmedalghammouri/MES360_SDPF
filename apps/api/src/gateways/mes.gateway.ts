...
@WebSocketGateway({
  cors: {
    ...
    // the mes360 / industry360 domain family, not just CORS_ORIGINS.
...
export class MesWebSocketGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      client.emit('connected', {
        message: 'Connected to MES360° real-time feed',
   ...
   * Data Streams update live. Topics: `mes360/<factoryId>/<machineKey>/<tagCode>`
  ...
  handleTagValue(payload: { topic: string; value: unknown; quality?: string; timestamp?: Date | string }) {
    ...
    if (parts[0] !== 'mes360' || parts.length < 4) return;
    ...
    // tag topic: mes360/<factory>/<machineKey>/<tagCode>
