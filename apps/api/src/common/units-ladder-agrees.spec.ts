 ...
 * `units.util.ts`. The obvious move was to lift it into `@mes360/shared` and
 * import it from both.
 *
 ...
 *
 * So the gateway has its copy in `@mes360/shared` — which pkg CAN bundle — and
 ...
 * duplicate, import `@mes360/shared` in both, and delete this file with it.
 */
describe('the two copies of the packaging ladder agree', () => {
  /** Real NCC master data, plus a ladder where every rung differs so a ×1 cannot hide a bug. */
  const PACKAGINGS = [
