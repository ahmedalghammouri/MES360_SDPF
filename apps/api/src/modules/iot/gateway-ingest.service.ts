 ...
 *  - `mes360/<factory>/jo/<jobOrderId>/count`  → roll counts up to the Work Order
 *  - `mes360/<factory>/energy/<meterId>`       → enrich the EnergyReading (WO/WorkCenter/state) + anomaly check
...
export class GatewayIngestService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    try {
      this.mqtt.subscribeToTag('mes360/+/jo/+/count', () => undefined);
      this.mqtt.subscribeToTag('mes360/+/energy/+', () => undefined);
