...
/**
 * Live MQTT monitor — subscribes to the whole `mes360/#` namespace so the in-app
...
export class MqttMonitorService implements OnModuleInit {
  ...
  onModuleInit() {
    try {
      ...
      this.mqtt.subscribeToTag('mes360/#', () => undefined);
      this.logger.log('MQTT monitor subscribed to mes360/#');
    } catch (err) {
