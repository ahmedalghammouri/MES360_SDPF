...
describe('KpiService.lineOeeAnalytics', () => {
  ...
  const machines = [machine('m1', 'Big Betti', 1), machine('m2', 'Cartomac', 2), machine('m3', 'Palletizer', 3)];
  ...
  describe('bottleneck is a single machine', () => {
    it('marks exactly one machine as the bottleneck', async () => {
      ...
      expect(r!.machines.find((m) => m.isBottleneck)!.name).toBe('Cartomac');
    });
  ...
  describe('final outfeed accepts several machines', () => {
    it('pools quality across every nominated outfeed', async () => {
      ...
      expect(r.basis.outfeedMachineNames).toEqual(['Cartomac', 'Palletizer']);
...
/**
 * Window clamping — found while verifying real NCC data.
 *
    ...
    const service = build(
      {
        ...
        machines: [machine('m1', 'Big Betti', 1)],
      },
      [{
