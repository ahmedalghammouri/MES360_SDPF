 ...
 *
 * For the SIDCO factory it creates:
...
export async function seedIotEnergyDemo(prisma: PrismaClient): Promise<void> {
  ...
  console.log(`[iot-energy-demo] SIDCO: ${machines.length} PLCs + counters, ${machines.length} machine meters${line ? ' + 1 line meter' : ''} (gateway "Plant Edge Gateway").`);
}
