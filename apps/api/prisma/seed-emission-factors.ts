// ============================================================
// MES360° — Scope 2 grid emission factor bootstrap
// ------------------------------------------------------------
...
//
// NCC supplied 0.568 kg CO2e/kWh for the KSA national grid.
//
// IDEMPOTENT AND NON-DESTRUCTIVE — this runs on EVERY boot:
//   • factory with no factor at all → insert the NCC baseline
...
/** NCC-supplied factor for the KSA national grid, used as the PoC baseline. */
const KSA_GRID_FACTOR = 0.568;
const SOURCE = 'NCC-supplied KSA national grid emission factor (PoC baseline)';
  ...
  for (const f of factories) {
    ...
    await prisma.gridEmissionFactor.create({
      data: {
        ...
        isActive: true,
        notes:
          'Seeded baseline. Confirm with NCC whether 0.568 is fixed for the PoC or should ' +
