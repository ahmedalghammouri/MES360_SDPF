// ============================================================
// MES360° — machine status signals  (Action Tracker ID 2, 10, 11)
// ------------------------------------------------------------
// Binds the DIGITAL INPUT signals NCC made available on the remote Modbus I/O.
//
...
//
// ── NCC's signal mapping, verbatim ──────────────────────────────────────────
//
// Big Betti I/O module:
//   ID 3  Big Betti — Run Mode
...
// Cartomac and Uni-Tech keep their Run Mode signal ON while starved. A starved
...
/**
 * NCC's I/O IDs are 1-based terminal numbers; Modbus addresses are 0-based.
 *
 *   ID 1 → DI 0 · ID 2 → DI 1 · ID 3 → DI 2 · …
 *
 * Confirmed with NCC. The offset is applied in one place rather than written out
 * per signal, so the table below stays readable against their document and cannot
 * drift from it row by row.
 *
 * The mapping corroborates itself: on the Big Betti module IDs 1–2 are that
...
interface SignalSpec {
  ...
  /** NCC's own words, stored so the interpretation travels with the tag. */
...
const SIGNALS: SignalSpec[] = [
  {
    ...
    code: 'M1_RUN_MODE', name: 'Big Betti — Run Mode',
    signalRole: 'RUN_MODE',
  ...
  },
  {
    ...
    code: 'M3_RUN_MODE', name: 'Cartomac — Run Mode',
    signalRole: 'RUN_MODE',
  ...
  for (const s of SIGNALS) {
    ...
    await prisma.tagDefinition.create({
      data: {
        ...
        code: s.code,
        name: s.name,
        description: `NCC I/O ID ${s.ioId}. ${s.interpretation}`,
    ...
    console.log(`  + ${s.code}: created → ${s.deviceName} DI${diAddress(s.ioId)} (NCC ID ${s.ioId}, ${s.signalRole})`);
    created++;
  }
