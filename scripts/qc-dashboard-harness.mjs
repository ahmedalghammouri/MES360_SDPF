...
// Requires: docker stack running (mes-api-plocal @ :3001, mes-postgres-plocal).
// ============================================================
import { execSync } from 'node:child_process';

const API = 'http://localhost:3001/api/v1';
const PG = 'mes-postgres-plocal';
const DB = 'mes360';
...
function sql(q) {
  const out = execSync(`docker exec ${PG} psql -U mes_user -d ${DB} -tAc "${q.replace(/"/g, '\\"')}"`, { encoding: 'utf8' });
  return out.trim();
}
...
async function run() {
  ...
  const faEmail = 'factoryadmin1_sdpf@mes360.sa';
  let token = await login(faEmail, `${faEmail}@${faEmail}`);
  if (!token) { token = await login('admin@mes360.sa', 'admin@mes360.sa@admin@mes360.sa'); rec('Auth', 'factory-admin login', 'WARN', 'fell back to super-admin (unscoped)'); }
