...
WO="${WO:-WO-2026-0005}"
EMAIL="${EMAIL:-admin@mes360.sa}"
PASSWORD="${PASSWORD:-admin@mes360.sa@admin@mes360.sa}"
...
if [ -z "$TOKEN" ]; then
  echo "Login failed. Is the api container up? Try:  docker logs mes-api-plocal --tail 20"
  exit 1
fi

WO_ID=$(docker exec -i mes-postgres-plocal psql -U mes_user -d mes360 -At \
